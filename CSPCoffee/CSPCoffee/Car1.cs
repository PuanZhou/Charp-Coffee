    ...
    public partial class Car1 : Form
    {
        ...
        private void LoadCouponToListbox()
        {
            var q = from c in db.CouponDetails
                    where c.MemberID == memID_p
                    select new { c.CouponID, c.Coupon.CouponName, c.Coupon.Money,c.Coupon.Condition };

            foreach (var n in q)
            {
                listBox1.Items.Add($" {n.CouponName} : 可折抵 {n.Money.ToString("#0.")} 元 (需滿{n.Condition}元) ");
        ...
        private void LoadAddproducts()
        {
            ...
            {                
                ...
                {
                    y[i] = new PDcontrol(qr[i], memID_p);
                    flowLayoutPanel2.Controls.Add(y[i]);
                    y[i].Margin = new Padding(35, 20, 35, 20);
                    y[i].Tag = qr[i];
                    y[i].theClick += Car1_theClick1;
                    y[i].button3.Visible = true;
                    y[i].button1.Visible = y[i].button2.Visible = false;
                }
        ...
        private void Car1_theClick1(PDcontrol source)
        {
            ...
            bool qe = db.ShoppingCarDetails.Any(n => n.MemberID == memID_p && n.ProductsID == (int)source.Tag);
            if (qe)
            {
                var qcar = db.ShoppingCarDetails.FirstOrDefault(x => x.ProductsID == (int)source.Tag && x.MemberID == memID_p);
                var q = db.ShoppingCarDetails.Where(x => x.ProductsID == (int)source.Tag && x.MemberID == memID_p).ToList();
                q[0].Quantity = qcar.Quantity + 1;
                db.SaveChanges();
            }
            else
            {
                var q = new ShoppingCarDetail { MemberID = memID_p, ProductsID = (int)source.Tag, Quantity = 1 };
        ...
        {           
            ...
            if (int.Parse(CouponName.ToString()) != 0)
            {
                Order order = new Order { MemberID = memID_p, OrderDate = dt, OrderStateID = 1, PaymentID = PayID, OrderAddress = label3Add.Text, CouponID = int.Parse(CouponID) };
                this.db.Orders.Add(order);
            }
            else if (int.Parse(CouponName.ToString()) == 0)   //沒有使用coupon
            {
                Order order1 = new Order { MemberID = memID_p, OrderDate = dt, OrderStateID = 1, PaymentID = PayID, OrderAddress = label3Add.Text };
                this.db.Orders.Add(order1);
            }
            this.db.SaveChanges();
            #endregion
            ...
            var d = db.ShoppingCarDetails.Where(p => p.MemberID == memID_p).Select(p => p);
            ...
            #region Update Stock
            var q2 = from o in db.ShoppingCarDetails
                     where o.MemberID == memID_p
                     select new { id = o.ProductsID, Quan = o.Quantity };

            List<object> P1 = new List<object>();
            List<object> Q1 = new List<object>();
            ...
            for (int i = 0; i < P1.Count(); i++)
            {
                int Pid = (int)P1[i];
                int Qid = (int)Q1[i];
                var q3 = db.Products.Where(p => p.ProductID == Pid).FirstOrDefault();
                if (q3 == null) return;
                q3.Stock = (int)q3.Stock - Qid;
            }
            db.SaveChanges();
            #endregion
